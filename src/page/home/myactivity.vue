    ...
    <div class="fly-panel fly-panel-user">
        <div class="layui-tab layui-tab-brief" lay-filter="user">
            <ul class="layui-tab-title" id="LAY_mine" :class="{'layui-tab-more':!ismore}">
                <li data-type="mine-jie" lay-id="index" :class="{'layui-this':tab==0}" @click="tab=0;publishEvent()"> {{ $t("my.activity") }}（{{mytotal}}）</li>
                <li lay-id="collection" :class="{'layui-this':tab==1}" @click="tab=1;collectEvent()">{{ $t("my.joinActivity") }}（{{mytotal1}}）</li>
                <li><router-link to="/addActivity">{{ $t("add.publishAct") }}</router-link></li>
                                    ...
                                    <tr v-for="(item,index) in topicDate">
                                        ...
                                        <td data-field="4" data-content="">
                                            <div class="layui-table-cell laytable-cell-1-4">
                                                ...
                                                @click="edit(item.id)">{{ $t("common.edits") }}</a>
                                                <a class="layui-btn layui-btn-sm" href="javascript:;"
                                                   @click="cancel(item.id)" style="background:#FF5722" v-if="item.cancel==1">{{ $t("common.cancle") }}</a>
                                            </div>
...
//           取消
            cancel(id){
                var me=this;
                me.$axios.post(me.url+"/activity/cancel?id="+id)
...
//           我发表的活动
            publish(){
                var me=this;
                me.$axios.post(me.url+"/activity/my?page=" + this.pageNum + "&size=" + this.pageSize+"&id="+JSON.parse(sessionStorage.getItem('vuex')).user.id)
                    .then(function (result) {
...
//          我参与的活动
            collect(){
                var me=this;
                me.$axios.post(me.url+"/activity/myjoin?page=" + this.pageNum + "&size=" + this.pageSize+"&id="+JSON.parse(sessionStorage.getItem('vuex')).user.id)
                    .then(function (result) {
