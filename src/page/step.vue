    ...
    <div class="fly-panel fly-panel-user" pad20>
      <div class="layui-tab layui-tab-brief">
        <ul class="layui-tab-title">
          <li class="layui-this">{{$t('forget.yanzheng')}}</li>
        </ul>
          <div class="layui-form layui-tab-content" id="LAY_ucm" style="padding: 20px 0;">
              <div class="layui-tab-item layui-show">
                  <div class="layui-form layui-form-pane">
                      ...
                      <el-form-item :label="$t('forget.sjh')" prop="mobile">
                        <el-input v-model="ruleForm.mobile"  type="text" :placeholder="$t('forget.phone')"></el-input>
                          ...
                          <el-form-item :label="$t('forget.sjyzm')" prop="yzm">
                              <el-input v-model="ruleForm.yzm"  type="text" placeholder="验证码" :style="{width:'75px'}"></el-input>
                      ...
                      <el-button type="primary" @click="submitForm('ruleForm')" :disabled="disShowGo" >{{$t('forget.step')}}</el-button>
                      </el-form>
                  </div>
              </div>
          </div>
...
<script>
  export default {
    ...
    methods: {
        getCode(){
            var me=this;
            me.$axios.post(me.url+"/user/sendregphone?phone="+this.ruleForm.mobile)
                .then(function (res) {
        ...
        submitForm(formName) {
            var me=this;
            me.$axios.post(me.url+"/user/getregphone?phone="+this.ruleForm.mobile+"&code=" + this.ruleForm.yzm)
                .then(function (res) {
                    if(res.data.result==true){
                        me.$message.success(res.data.msg);
                        sessionStorage.setItem('tel',me.ruleForm.mobile)
